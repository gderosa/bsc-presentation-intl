...
sub preamble {
    print GNUPLOT "reset\n";
    if ($sizex and $sizey) {
        print GNUPLOT "set terminal png small size $sizex, $sizey\n";
#        print GNUPLOT "set terminal png crop small size $sizex, $sizey\n";
    } else {
        print GNUPLOT "set terminal png small\n";   
...
sub videncode {
    ...
    my $mpeghq          = "$videodir/$name-hq.mpg";
    my $mp4             = "$videodir/$name.mp4";
    my $mp4lq           = "$videodir/$name-lq.mp4";
    my $mp4hq           = "$videodir/$name-hq.mp4";
    my $avi             = "$videodir/$name.avi";
    my $flv             = "$videodir/$name.flv";

    my $ogglqopts   = "-v 2.5";    # low quality/bitrate option for OGG/Theora
    my $mp4lqopts   = "-b 190k";   # low quality/bitrate option for MP4
    my $mpeglqopts  = "-b 160k";   # low quality/bitrate option for MPEG
 
    my $ogghqopts    = "-v 10";  # high q opts for OGG/Theora
    my $mp4lhopts    = "-sameq";    # high quality option for MP4
    my $mpeghqopts    = "-sameq";   # high quality option for MPEG

    # high quality without wasting bandwidth...
#   my $ogghqopts   = "-V 2500";    # high quality option for OGG/Theora
#   my $mp4hqopts   = "-b 2000k";   # high quality option for MP4
#   my $mpeghqopts  = "-b 2000k";   # high quality option for MPEG
...
#    # @different qualities
    system("$ffmpeg2theora -o $ogghq -S 0 $ogghqopts --optimize $lossless");
#
#    system("$ffmpeg2theora -o $ogglq -S 0 $ogglqopts --optimize $lossless"); 
#
#    # MPEG1 and MPEG4
#
#    system("$ffmpeg -sameq -y -i $lossless $mpeghq");
#    system("$ffmpeg -y $mpeglqopts -i $lossless $mpeglq");
#
#    system("$ffmpeg -y $mp4hqopts -i $lossless $mp4hq");
#    system("$ffmpeg -y $mp4lqopts -i $lossless $mp4lq");
#      
